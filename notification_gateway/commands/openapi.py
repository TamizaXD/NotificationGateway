...
import frappe
import click
from notification_gateway.hooks import CW_OPENAPI_OUTPUT_FILE

from notification_gateway.generate_swagger import write_openapi_spec
...
def generate_cw_openapi_spec():
    api_directory = frappe.get_app_path("notification_gateway", "api")
    output_file = get_output_file()
    write_openapi_spec(api_directory, output_file)


def get_output_file():
    output_file = frappe.get_app_path("notification_gateway", *CW_OPENAPI_OUTPUT_FILE)
    return output_file
