...
import os.path
import frappe
from notification_gateway.generate_swagger import generate_openapi_spec
import json
...
def get_context(context):
	openai_file_path = frappe.get_app_path("notification_gateway", *frappe.get_hooks("CW_OPENAPI_OUTPUT_FILE"))
	openapi_spec = None
	try:
		with open(openai_file_path, "r") as openai_file:
			openapi_spec = openai_file.read()
	except:
		api_directory = frappe.get_app_path("notification_gateway", *frappe.get_hooks("CW_OPENAPI_DIR"))
