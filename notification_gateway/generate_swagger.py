...
def generate_openapi_spec(api_dir):
    ...
    app_path = frappe.get_app_path("notification_gateway")
            ...
            if file.endswith(".py"):
                ...
                for func_name, details in docstring_and_args.items():
                    if details["docstring"]:
                        # openapi_spec["paths"][f"/api/v2/method/Message Service/send_message{method_path}.{func_name}"] 
                        openapi_spec["paths"][f"/api/method/notification_gateway.{method_path}.{func_name}"] = {
                            "post": {
